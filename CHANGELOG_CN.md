...
### 🐛 修复 — picker 拦截不再输给手速快的消息

0.4.1 的拦截有个洞：它只在 statusPolling 已经把 picker 记录为打开状态
（`interactiveModes`）之后才生效，而这个状态最多要等一个轮询 tick（约 2 秒）
**之后**才写入。`/model` 和下一句话连着发，下一句正好落进这个空窗 ——
拦截被跳过，文本被 picker 吞掉，末尾的回车还确认了高亮的模型。
和 0.4.1 修的是同一个事故,只是隔了一条街。

现在拦截**对每条入站消息都做新鲜的 `capturePane` 判断**,完全不依赖
内存状态 —— 哪怕 picker 刚渲染出来 1 毫秒也能拦住。代价是每条消息
一次 tmux capture（约 10 毫秒）。同时把真实的 `/model` picker 界面
固化成解析器测试 fixture：以后 Claude Code 改 UI 会让测试先红,
而不是拦截静默失效。

## 0.4.1 — 2026-06-10

### 🐛 修复 — 入站消息不再被打开的 TUI 选择器吞掉

终端里有交互式选择器打开时（`/model`、`AskUserQuestion`、权限确认框等），
从 Telegram 发的消息会被原样 sendKeys 进 pane —— 文本被选择器当作按键吃掉，
末尾的回车还会确认当前高亮项。真实事故：`/model` 选择器打开期间发的消息
悄悄选中了一个模型，文本本身石沉大海。权限确认框更危险 —— 泄漏进去的
`y`/数字键可能直接误批准操作。

现在选择器确认处于打开状态时，bot 会拒发普通文本（和图片）并回复明确提示
—— 消息留在 Telegram 里，不会消失。拦截前会用新鲜的 `capturePane` 复核一次,
避免选择器状态滞后一个轮询 tick 造成的误拦。

需要往选择器里打字时（`AskUserQuestion` 的 "Other" 输入框、`/model` 的过滤),
消息以 `>` 开头即可 —— 其余内容会**不带回车**打进 pane,再用选择器键盘上的
⏎ 按钮确认,绝不会误触发选中。
